...
import { AlertTriangle, AlertCircle, Info, ArrowLeft, Calendar, MapPin, Building } from 'lucide-react'
...
export default function RecallsPage() {
  ...
  const fetchRecalls = async () => {
    ...
    try {
      const url = filter === 'all'
        ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/recalls?limit=50`
        : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/recalls?classification=${filter}&limit=50`
  ...
  const filteredRecalls = recalls.filter(recall =>
    searchQuery === '' ||
    recall.product_description.toLowerCase().includes(searchQuery.toLowerCase()) ||
    recall.company_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    recall.reason_for_recall.toLowerCase().includes(searchQuery.toLowerCase())
  )
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`px-4 py-3 rounded-lg font-medium transition-colors ${
                  filter === 'Class I' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                Critical
              </button>
              <button
                ...
                className={`px-4 py-3 rounded-lg font-medium transition-colors ${
                  filter === 'Class II' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                High
              </button>
              <button
                ...
                className={`px-4 py-3 rounded-lg font-medium transition-colors ${
                  filter === 'Class III' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                Moderate
              </button>
              ...
              <div key={recall.id} className="bg-white rounded-xl shadow-sm border-l-4 border-red-500 p-6 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    recall.status === 'Ongoing' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
                  }`}>
                    {recall.status}
                  </span>
                </div>
